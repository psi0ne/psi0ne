# 🤓 About Me
Passionate back-end developer.
<br/>
<br/>
<h1 align="center">Tech Stack & Tools</h1>
<br/>
<p align="center">
   <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?&i=python,js,php,nodejs,solidity,bots,fastapi,jquery,docker,ubuntu,nginx,firebase,github,gmail,instagram,ipfs,kali,linkedin,mongodb,postman,powershell,rabbitmq,redis,regex,twitter,vercel,vscode,windows,apple,swift" />
  </a>
  <br/>
  <br/>
  <img src="https://komarev.com/ghpvc/?username=psi0ne&color=blueviolet"/>
</p>

# Stats
<img align="center" src="https://github-readme-stats.vercel.app/api?username=psi0ne&show_icons=true&theme=radical"> 
<img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=psi0ne&layout=compact&theme=radical">
